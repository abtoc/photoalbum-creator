...
trait MakeAlbum
{
    ...
    public function make(Album $album)
    {
        ...
        chdir($path);
        $command = '/usr/bin/zip -0 -X '.$path.'.epub mimetype'; 
        Log::info('Making epub file. album_id='.$album->id.',path='.$path.'.'.'epub,command='.$command);
        $result = exec($command, $output);
        Log::info('Maked epub file. album_id='.$album->id.',path='.$path.'.epub,command='.$command.',result='.$result.':'.implode(PHP_EOL, $output));
        $command = '/usr/bin/zip -r '.$path.'.epub * -x mimetype'; 
        Log::info('Making epub file. album_id='.$album->id.',path='.$path.'.epub,command='.$command);
        $result = exec($command, $output);
        Log::info('Maked epub file. album_id='.$album->id.',path='.$path.'.epub,command='.$command.',result='.$result.':'.implode(PHP_EOL, $output));
