    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route_query('admin.news.store') }}" method="POST">
                        @csrf
                            ...
                            <div class="col-md-10">
                                <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                                    @foreach(App\Enums\News\Status::cases() as $status)
                                        <option value="{{ $status->value }} @selected(!is_null(old('status')) && $status->value === old('status'))">
                                            {{ $status->string() }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-md-10">
                                <textarea name="message" id="message" cols="30" rows="20" class="form-control @error('title') is-invalid @enderror" required>{{ old('message') }}</textarea>
